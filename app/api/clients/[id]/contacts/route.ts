...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    const data = await fetchContacts(params.id, window);
    // Don't expose raw contact PII in the response — return aggregates only
    const { contacts: _contacts, ...summary } = data;
